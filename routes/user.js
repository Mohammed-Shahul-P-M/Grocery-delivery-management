...
function veryfyUser(req, res, next) {
  if (req.session.userId && req.session.userData) next();
    ...
    foundUrl ? res.json({ loginErr: true }) : res.redirect("/login");
  }
}
// home route for user
router.get("/", async (req, res) => {
  ...
  (req.session.userData && req.session.userData.profilenotUploaded)
    ? res.redirect("/profile")
    : res.render("user/home", { user, userData: req.session.userData, allStore });
});
// route for login
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    let { phone, password } = req.body // only to fetch if the name manipulated
...
router.get("/feedback/:id", veryfyUser, async (req, res) => {
  ...
  if (req.params.id.length == 24) { // checking the length of the id to avoid mongodb error
...
// router to post feedback of user 
...
router.get("/download/:location/:id", veryfyUser, async (req, res) => {
  ...
  if (req.params.id.length !== 24) return res.json('Unauthorized')
...
// route to provide required data
...
router.post("/addTocart", veryfyUser, (req, res) => {
  let storeId = req.headers.referer.split("/")[4];
  let action = null;
  let prodIndex = null;
...
router.post("/register", async (req, res) => {
  ...
  if (reqData.resend) {
    ...
    const options = {
      method: "post",
      headers: {
        Authorization: 'Token ' + process.env.otpToken,
